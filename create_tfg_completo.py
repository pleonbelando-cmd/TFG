...
def P(doc, text, cites=None):
    """Párrafo normal con soporte a **negrita**, *cursiva* y superíndices de cita."""
    import re
    p = doc.add_paragraph()
    set_spacing(p)
    ...
    for tok in tokens:
        if tok.startswith("**") and tok.endswith("**"):
            ...
            p.add_run(tok)
    if cites:
        cite_str = ",".join(str(n) for n in cites)
        r = p.add_run(f"[{cite_str}]")
...
def cap2(doc):
    ...
    P(doc, "El problema fundamental de los MES es que esas restricciones de identificación no pueden verificarse a partir de los datos. Como señaló Sims (1980) en su influyente crítica, las restricciones de identificación son *increíbles* porque se imponen por razones de conveniencia estadística, no por razones económicas sólidas. La propuesta de Sims fue el modelo **VAR** (*Vector Autoregression*), que trata todas las variables del sistema como igualmente endógenas y modela cada una como función de los retardos de todas las demás —la **forma reducida** del MES sin restricciones de identificación arbitrarias.", cites=[25])
...
def cap3(doc):
    ...
    P(doc, "Baur y McDermott (2010) cuantificaron este efecto distinguiendo entre el rol de *hedge* del oro —presente en la muestra completa— y su rol de *safe haven* —presente específicamente en los quintiles de peores retornos del mercado de renta variable. El VIX captura precisamente esos episodios de cola negativa del mercado, por lo que es una variable especialmente informativa para el análisis del oro como activo de refugio.", cites=[3])
...
def cap5(doc, figs):
    ...
    P(doc, "Con las cuatro variables I(1) identificadas, se aplica el test de cointegración multivariante de Johansen (1991) al sistema {ln(Oro), ln(DXY), TIPS, ln(S&P 500)} con constante dentro del vector de cointegración.", cites=[17,18])
    TABLE(doc,
    ...
    P(doc, "El test ARCH-LM rechaza la ausencia de heterocedasticidad condicional en los retornos del oro (p < 0,05), justificando un análisis complementario de volatilidad. Se estima un modelo **GJR-GARCH(1,1) con distribución t de Student** que captura la posible asimetría en la respuesta de la volatilidad ante shocks positivos y negativos (parámetro γ).", cites=[11,13])
    TABLE(doc,
...
def cap7(doc, figs):
    ...
    P(doc, "La validación cruzada estándar (*k-fold*) introduce *look-ahead bias* en series temporales al barajar las observaciones: el modelo podría entrenarse con datos de 2022 para predecir 2015. La **validación walk-forward con ventana expandible** elimina este problema: el modelo se entrena en [1, t-1] y predice t; a continuación, amplía el entrenamiento a [1, t] y predice t+1, sin incluir nunca información posterior al instante de predicción. El reentrenamiento completo se realiza cada 3 pasos para XGBoost y cada 6 para Random Forest y LSTM, acotando el coste computacional.", cites=[20])

    H2(doc, "7.3 Descripción de los tres modelos")
    P(doc, "**XGBoost** construye árboles de decisión secuencialmente, donde cada árbol corrige los errores del anterior (*boosting*). La configuración es conservadora —profundidad máxima 3, tasa de aprendizaje 0,05, regularización L1 y L2— para evitar sobreajuste con solo 162 observaciones iniciales.", cites=[6])
    P(doc, "**Random Forest** construye 300 árboles en paralelo, cada uno sobre un subconjunto aleatorio de observaciones y variables (*bagging*). La decorrelación entre árboles reduce la varianza del conjunto, haciéndolo especialmente robusto en muestras pequeñas (n < 500).", cites=[4])
    P(doc, "**LSTM** (*Long Short-Term Memory*) es una red neuronal recurrente que procesa secuencias temporales de 6 meses y aprende qué información retener y cuál olvidar mediante compuertas internas. La arquitectura es intencionadamente simple (32 unidades, 1 capa) para evitar sobreajuste. Se implementa *early stopping* con paciencia de 20 épocas.", cites=[16])
...
def referencias(doc):
    ...
    refs = [
        "[1] Barsky, R. B., & Summers, L. H. (1988). Gibson's paradox and the gold standard. *Journal of Political Economy, 96*(3), 528–550.",
        ...
        "[5] Brown, R. L., Durbin, J., & Evans, J. M. (1975). Techniques for testing the constancy of regression relationships over time. *Journal of the Royal Statistical Society, Series B, 37*(2), 149–163.",
        "[6] Chen, T., & Guestrin, C. (2016). XGBoost: A scalable tree boosting system. *Proceedings of the 22nd ACM SIGKDD*, 785–794.",
        "[7] Chicago Fed. (2021). *What drives gold prices?* Chicago Fed Letter, No. 464.",
        ...
        "[10] Driscoll, J. C., & Kraay, A. C. (1998). Consistent covariance matrix estimation with spatially dependent panel data. *Review of Economics and Statistics, 80*(4), 549–560.",
        "[11] Engle, R. F. (1982). Autoregressive conditional heteroscedasticity. *Econometrica, 50*(4), 987–1007.",
        "[12] Erb, C. B., & Harvey, C. R. (2013). The golden dilemma. *Financial Analysts Journal, 69*(4), 10–42.",
        "[13] Glosten, L. R., Jagannathan, R., & Runkle, D. E. (1993). On the relation between the expected value and the volatility of the nominal excess return on stocks. *Journal of Finance, 48*(5), 1779–1801.",
        ...
        "[18] Johansen, S., & Juselius, K. (1990). Maximum likelihood estimation and inference on cointegration. *Oxford Bulletin of Economics and Statistics, 52*(2), 169–210.",
        "[19] Liang, C., Li, Y., et al. (2023). Forecasting gold price using machine learning methodologies. *Chaos, Solitons & Fractals, 173.*",
        ...
        "[23] O'Connor, F. A., Lucey, B. M., Batten, J. A., & Baur, D. G. (2015). The financial economics of gold — a survey. *International Review of Financial Analysis, 41*, 186–205.",
        "[24] Plakandaras, V., et al. (2022). Forecasting the price of gold using machine learning methodologies. *Applied Economics, 54*(33), 3768–3783.",
        "[25] Sims, C. A. (1980). Macroeconomics and reality. *Econometrica, 48*(1), 1–48.",
        "[26] Wooldridge, J. M. (2007). *Introducción a la econometría: un enfoque moderno* (3.ª ed.). Thomson.",
        "[27] World Gold Council. (2023). *Gold Demand Trends: Full Year 2023.* World Gold Council.",
        "[28] World Gold Council. (2024). *Gold Demand Trends: Full Year 2024.* World Gold Council.",
    ]
    for ref in refs:
